...
vector<int> cota(vector<vector<int> > allegados, int N, int M){
    ...
    for (int i = M - 2; i >= 0; --i){
       opt[i] = num_allegados[i + 1] + opt[i + 1];  
    }
...
pair<int, bool > resolver(vector<vector<float> > distancias, vector<vector<int> > allegados, int N, int M){
    ...
    while (!C.empty() && C.top().parejas_opt >= parejas_max){
        Y = C.top(); C.pop();
...
void resuelveCaso() {
    ...
    pair<int, bool> sol = resolver(distancias, allegados, N, M);

    if (M == 0) cout << "PAREJAS 0\n";
    else {
       if (sol.second) cout << "PAREJAS " << sol.first << '\n';
       else cout << "CANCELA\n";
    }
